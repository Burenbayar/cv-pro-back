...
function mergeParsed(text: string, skills: string[], rawCvText?: string): ParsedCv {
  const raw = (rawCvText || text).trim();
  return enrichParsedCv(parseCvSections(text), raw, skills);
}

function sectionTitle(doc: any, x: number, y: number, w: number, title: string, color: string) {
...
function bullets(doc: any, x: number, y: number, w: number, items: string[]) {
  let cy = y;
...
function drawHeader(doc: any, name: string, role: string, summary: string, contact: ReturnType<typeof parseContactFields>, photo: Buffer | null, pageW: number, L: ReturnType<typeof templateLabels>) {
  const top = M;
  const w = pageW - M * 2;
...
export function createProfessionalCvPdf(input: CvPdfInput): Promise<Buffer> {
  ...
  const parsed = sanitizeParsedForTemplate(
    mergeParsed(normalizedCv, input.skills, normalizedRaw),
    normalizedRaw,
    input.skills,
  );
  const name =
  ...
  if (parsed.education.length) {
    ...
    for (const line of parsed.education) {
      ...
      if (edu.date) doc.fillColor(CV_THEME.muted).fontSize(8.5).text(edu.date, mainX + mainW - 68, my, {width: 68, align: 'right'});
      my = doc.y + 2;
      if (edu.detail) {
